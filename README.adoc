...
generated; it'll be required when you run the setup wizard in the next step.

==== Settings

The recommended way to configure the system is to run the provided setup wizard
...
you can populate the templates by hand.
...
* Maximum spot price determines the maximum price you're willing to pay per
  ...
  bill at the end of the month. Defaults to `0.05`.
* The server file name must exactly match the name of your Minecraft archive,
  see below.
...
Update `ansible/ec2.ini`'s `regions` entry with the AWS region you're using.
You can set this to _all_, but the dynamic EC2 entry script will take longer.
This tells the auto-inventory script what region to check for instances.
